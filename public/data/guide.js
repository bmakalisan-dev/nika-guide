// このファイルが文面のすべて。ここだけ書き換えればページが変わる。

const GUIDE = {
    site: {
        title: "ニカ 編成講座",
        lede: "部隊は3体で1つ。無駄を作らず、方向性を決めて、色で有利を取る。それだけの話を図で確認する。",
        gameName: "ニカ",
        gameUrl: "", // 例: https://example.com/nika
    },

    // 冒頭。このサイトの立ち位置と読み方。
    policy: {
        head: "このサイトの方針",
        body: "（ここに方針を書く）",
        points: [
            "（箇条書きが要るならここに。不要なら points を空配列にする）",
        ],
    },

    // 部隊の構造を1枚で見せる図。
    overview: {
        head: "部隊の見取り図",
        monsters: ["白虎", "スレイプニル", "阿修羅"],
        callouts: [
            { label: "枠は3つ", body: "1部隊はモンスター3体。ここが編成のすべて。" },
            { label: "HPと防御は合算", body: "3体のHPと防御力は合算して1つとして扱う。1体だけ硬くしても意味がない。" },
            { label: "色は多数決", body: "部隊の属性は3体の多数決で決まる。3色では編成できない（太陽タロットを除く）。" },
        ],
    },

    principles: {
        head: "編成の三か条",
        items: [
            {
                no: "第一条",
                head: "重複不可の能力は1体まで",
                body: "重複不可の能力は2体以上入れても効果が増えない。さらにその能力を持つモンスターはステータスが低めに設定されているため、重ねた分だけ部隊が弱くなる。",
                figure: "stack",
            },
            {
                no: "第二条",
                head: "方向性を決める",
                body: "速度・防御・攻撃・クリティカルなど、どれか1つに寄せた部隊が強い。スキルとタロットを選びやすくなり（迅速・猛攻・鉄壁）、能力同士の相乗効果も狙える（攻撃力上昇と威圧）。",
                figure: "types",
            },
            {
                no: "第三条",
                head: "色で有利を取る",
                body: "（仮）部隊の属性は3体の多数決で決まる。赤→緑→青→赤の有利を取ると攻撃・防御・速度すべてに補正が乗る。最低2体は同じ色で固める。",
                figure: "triangle",
            },
        ],
    },

    types: {
        head: "タイプ別 Tierと編成",
        note: "Tierは特殊能力の評価。編成例はその型の組み方の見本。",
        items: [
            {
                id: "speed",
                label: "速度型",
                accent: "blue",
                lead: "先に殴る回数で押し切る型。行動タイマーを縮める能力を重ねる。",
                tiers: [
                    { rank: "S", abilities: ["疾風", "急襲", "指揮"] },
                    { rank: "A", abilities: ["連撃", "精密攻撃"] },
                    { rank: "B", abilities: ["飛翔", "水陸両用"] },
                ],
                skills: ["迅速", "攻城"],
                tarots: [{ name: "星", position: "正" }],
                formations: [
                    { name: "青速攻", monsters: ["鳥人間", "キラーシャーク", "ボス猿"], aim: "疾風で全員の初手を早め、急襲で追撃の波を作る。" },
                    { name: "赤速攻", monsters: ["白虎", "スレイプニル", "阿修羅"], aim: "街解放後の完成形。連撃で速度が伸び続ける。" },
                ],
            },
            {
                id: "defense",
                label: "防御型",
                accent: "green",
                lead: "殴られながら固くなる型。長期戦に持ち込んで勝つ。",
                tiers: [
                    { rank: "S", abilities: ["守護", "甲殻"] },
                    { rank: "A", abilities: ["大盾Ⅰ", "ダメージ減少", "見切り"] },
                    { rank: "B", abilities: ["カウンター", "強気", "再生"] },
                ],
                skills: ["鉄壁"],
                tarots: [{ name: "皇帝", position: "正" }],
                formations: [
                    { name: "青甲殻", monsters: ["玄武", "アルマジロ", "マジックポット"], aim: "甲殻を重ねて防御力を積み上げ、守護で立ち上がりを凌ぐ。" },
                    { name: "緑耐久", monsters: ["ユグドラシル", "シールダー", "ゾンビ"], aim: "軽減と再生でHPを保たせ、相手の手数を枯らす。" },
                ],
            },
            {
                id: "attack",
                label: "攻撃型",
                accent: "red",
                lead: "1発の重さで押す型。倍率が乗る能力を集める。",
                tiers: [
                    { rank: "S", abilities: ["号令", "狂戦士"] },
                    { rank: "A", abilities: ["突撃", "闘争心", "ダメージ追加"] },
                    { rank: "B", abilities: ["咆哮", "兜割り", "吸血"] },
                ],
                skills: ["猛攻", "撃滅"],
                tarots: [{ name: "愚者", position: "正" }],
                formations: [
                    { name: "赤猛攻", monsters: ["バーサーカー", "ゴブリントロッコ兵", "ワルキューレ"], aim: "狂戦士で部隊全体の攻撃を底上げし、突撃で倍率を伸ばす。" },
                    { name: "緑総攻撃", monsters: ["アヴェンジャー", "キラーマシーン", "バンシー"], aim: "号令の全体攻撃を狙い、咆哮のスタンで相手の反撃を止める。" },
                ],
            },
            {
                id: "critical",
                label: "クリティカル型",
                accent: "amber",
                lead: "クリティカル率と倍率を両方積む型。噛み合えば最大火力。",
                tiers: [
                    { rank: "S", abilities: ["車輪", "渾身"] },
                    { rank: "A", abilities: ["精密攻撃", "急襲"] },
                    { rank: "B", abilities: ["弱肉強食", "邪眼"] },
                ],
                skills: ["会心"],
                tarots: [{ name: "女帝", position: "正" }],
                formations: [
                    { name: "赤会心", monsters: ["火車", "赤鬼", "レッサーパンダ"], aim: "車輪でクリティカル率を回し、渾身で1発を伸ばす。" },
                    { name: "青会心", monsters: ["ハンター", "キラーシャーク", "サイクロプス"], aim: "クリティカルのたびに行動タイマーが進み、手数と会心が同時に増える。" },
                ],
            },
        ],
    },

    footer: {
        disclaimer: "当サイトは非公式のファン攻略ページです。ゲーム運営とは関係ありません。内容は個人の見解であり、ゲーム側の仕様変更で古くなることがあります。",
        credit: "モンスター画像は monta! 提供「ＲＰＧツクール２００３用 戦闘敵ユニット素材」を使用。",
    },
};
