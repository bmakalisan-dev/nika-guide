...
| タイプ別 Tierと編成 | タブ4つ（速度型・防御型・攻撃型・クリティカル型）。各タブに「相性が良い特殊能力」「使用頻度Tier」「相性が良いスキル・タロット」「代表的な編成」 |
| フッター | 免責文 |
...
- `types.items` … タイプごとの `traits`（相性が良い特殊能力の傾向）、`tiers`（rank と特殊能力名）、`skills`・`tarots`（相性が良いもの。名前だけを並べる）、`formations`（モンスター3体・狙い）。`accent` は `red` / `green` / `blue` / `amber`
